...
def checkout_view(request):
    ...
    for item in cart.items.all():
        order_item_price = item.product.price.to_decimal()
    ...
    # Send the invoice email to the console.
    subject = f'Your Burgundy Parcelle Order Invoice #{order.id}'
    message = render_to_string('shop/emails/invoice.html', {'order': order})
    send_mail(subject, message, 'noreply@burgundyparcelle.com', [request.user.email], fail_silently=False)
